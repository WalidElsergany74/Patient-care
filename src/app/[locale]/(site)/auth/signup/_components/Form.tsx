...
const Form = ({locale , translations} : {locale : Locale , translations : Translations}) => {
    ...
    useEffect(() => {
      if (state.status && state.message){
        ...
        if (state.status === 201) {
          ...
          toast.success(translations.messages.accountCreated)
        }
      }, [locale, router, state.message, state.status, translations.messages.accountCreated]);
