...
import { Pages, Routes } from "@/constants/index";
...
export default withAuth(
  async function middleware(request: NextRequest) {
    ...
    const protectedRoutes = [Routes.PROFILE, Routes.ADMIN, Routes.DOCTORADMIN,Routes.REVERSATION];
    ...
    if (isAuthPage && isAuth) {
      ...
      if (role === UserRole.DOCTOR) {
        return NextResponse.redirect(
          new URL(`/${currentLocale}/${Routes.DOCTORADMIN}`, request.url)
        );
      }
      return NextResponse.redirect(
    ...
    // if user loggedin and tries to access doctor admin route
if (
  isAuth &&
  pathname.startsWith(`/${currentLocale}/${Routes.DOCTORADMIN}`)
) {
  const role = isAuth.role;
  if (role === UserRole.PATIENT) {
    return NextResponse.redirect(
